...
in {
  ...
  services.octoprint = {
    ...
    extraConfig = {
      ...
      accessControl = {
        autologinLocal = true;
        autologinAs = "root";
        localNetworks = [ "127.0.0.0/8" "192.168.0.0/16" "100.0.0.0/8" ];
      };
  ...
  systemd.services.octoprint = {
    ...
    preStart = let
      ...
      cfgUpdate = pkgs.writeText "octoprint-users.yaml" (builtins.toJSON {
        _version = 2;
        root = {
          active = true;
          apikey = null;
          groups = [ "users" "admins" ];
          password =
            "$argon2id$v=19$m=65536,t=3,p=4$3FtLCWGM8Z5TCmHsXQuh9A$H5rEbKFN5O1zhn/E6q288KkDIOq4yTMTYyz4SsLPshQ";
          permissions = [ ];
          roles = [ "user" "admin" ];
          settings = { };
        };
  ...
  systemd.tmpfiles.rules = [
    ...
    "L+ ${config.services.octoprint.stateDir}/printerProfiles/${path} - - - - ${
      ./profiles + ("/" + path)
